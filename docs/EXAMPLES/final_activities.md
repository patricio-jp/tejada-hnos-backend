...
### 1. Reuniones e ingeniería de requisitos
  ...
  - **Estado:** ✅ COMPLETADO
  - **Duración:** 1 semana
  - **Descripción:** Serie de reuniones con el cliente Tejada Hnos. para definir y documentar requisitos funcionales y no funcionales del sistema. Identificación de actores (Administrador, Capataz, Operario), flujos de trabajo principales, y especificaciones de trazabilidad de productos desde compra de insumos hasta venta final. Elaboración de casos de uso y definición del alcance del proyecto.
...
### 27. F12: Módulo de trazabilidad E2E con visualización de flujo
  - **Estado:** 📦 FASE 2 - Planificado para versión futura
  - **Descripción:** Implementación de módulo de trazabilidad completa con visualización tipo flowchart interactivo (ReactFlow o similar). Buscador de lotes de cosecha que muestra flujo visual completo: origen (parcelas, actividades de cosecha, insumos utilizados en cultivo), destino (órdenes de venta asignadas, envíos realizados, clientes finales). Visualización gráfica con nodos interactivos clicables para expandir información. Vista inversa: dado un envío, mostrar toda la cadena de trazabilidad hacia atrás hasta las parcelas. Integración con endpoints E2E del backend. Interfaz para cumplimiento de normativas de trazabilidad alimentaria y auditorías. **Esta funcionalidad se implementará en la Fase 2 del proyecto tras la entrega del MVP.**

### 28. F13: Empaquetado multiplataforma con Electron y Capacitor
  - **Estado:** 📦 FASE 2 - Planificado para versión futura
  - **Descripción:** Configuración y empaquetado de la aplicación para múltiples plataformas. Setup de Electron para compilar aplicación de escritorio para Windows, macOS, Linux con instaladores nativos. Configuración de Capacitor para compilar aplicación móvil nativa para Android e iOS. Ajustes específicos de cada plataforma: íconos personalizados, splash screens, permisos de sistema (cámara para fotos en actividades, ubicación GPS). Pruebas de funcionalidad en cada plataforma target. Generación de instaladores (EXE, DMG, DEB) y APKs/IPAs firmados. Documentación de proceso de build y despliegue. **La entrega MVP se realizará como aplicación web SPA accesible desde navegadores modernos en cualquier dispositivo. El empaquetado multiplataforma se implementará en la Fase 2 según necesidades del cliente.**

### 17. F2: Autenticación, manejo de roles y ruteo protegido
  - **Inicio:** 14/02/2026
  - **Fin:** 21/02/2026
  - **Duración:** 1 semana
  - **Descripción:** Implementación del módulo de autenticación en frontend con React Context API para gestión de estado de sesión. Desarrollo de páginas de Login y Register con validación de formularios. Integración con endpoints de autenticación del backend (login, refresh token, logout). Almacenamiento seguro de tokens en localStorage con interceptores Axios para incluir tokens en headers. Sistema de ruteo protegido con React Router: rutas privadas que redirigen a login si no hay sesión. Implementación de componentes de protección por rol: ProtectedRoute que valida roles ADMIN, CAPATAZ, OPERARIO. Manejo de expiración de tokens con refresh automático. Componente de perfil de usuario. Documentación en `IMPLEMENTACION_F2.4_RUTEO_ROLES.md`.

### 18. F3: Interfaces CRUD para maestros de datos (Variety, Customer, Supplier, Input)
  - **Inicio:** 22/02/2026
  - **Fin:** 01/03/2026
  - **Duración:** 1 semana
  - **Descripción:** Desarrollo de interfaces completas para gestión de entidades maestras en módulos separados. Implementación de DataTables interactivas con paginación, búsqueda, ordenamiento, y filtros para Varieties, Customers, Suppliers, Inputs. Modales de creación y edición con formularios validados usando React Hook Form y Zod. Botones de acciones: crear, editar, eliminar (con confirmación). Integración con API REST del backend. Manejo de estados de carga, errores, y éxito con toast notifications. Componente reutilizable de tabla (DataTable) parametrizable. Filtros por estado activo/inactivo. Documentación en `IMPLEMENTACION-CUSTOMERS-COMPLETA.md`.

### 19. F4: Módulo de Compras con gestión de órdenes y recepciones
  - **Inicio:** 02/03/2026
  - **Fin:** 11/03/2026
  - **Duración:** 1.5 semanas
  - **Descripción:** Implementación del módulo completo de Purchases con múltiples vistas. Página de listado de órdenes de compra con DataTable filtrable por estado, fecha, proveedor. Vista de detalle de orden con tabla de ítems y estado de recepción (cantidad pedida vs. recibida). Formulario de creación de orden de compra con selección de proveedor, agregado dinámico de ítems (insumo, cantidad, precio), cálculo automático de totales. Módulo de registro de recepciones (GoodsReceipt) vinculado a órdenes, con posibilidad de recepción parcial o total. Indicadores visuales de estado: badges con colores según PENDING/PARTIAL/COMPLETED. Tracking visual de progreso de recepciones con progress bars. Documentación en `RESUMEN_PURCHASES.md`, `datatable-purchases.md`, y `alineacion-backend-frontend.md`.

### 20. F5: Módulo de Campos y Parcelas con mapas interactivos GeoJSON
  - **Inicio:** 12/03/2026
  - **Fin:** 21/03/2026
  - **Duración:** 1.5 semanas
  - **Descripción:** Desarrollo de módulos Fields y Plots con visualización geoespacial avanzada. Implementación de InteractiveMap con Deck.gl para renderizado de polígonos GeoJSON de campos y parcelas. Vista de lista de campos con tarjetas que muestran nombre, área, parcelas asociadas. Vista de detalle de campo con mapa interactivo mostrando todas sus parcelas coloreadas. Herramienta de edición de geometrías: PlotsEditor que permite dibujar, editar, y eliminar polígonos de parcelas sobre el mapa. Integración con Mapbox o OpenStreetMap para capa base. Selección interactiva de parcelas con highlighting. Formularios de creación/edición de campos y parcelas con validación de geometrías. Documentación extensa en `InteractiveMap.md`, `InteractiveMap-modos.md`, `guia-migracion-interactivemap.md`, `mejoras-interactivemap.md`, `mejoras-visuales-plotseditor.md`, y `ejemplo-visual-unidades.md`.

### 21. F6: Módulo de Órdenes de Trabajo para planificación (Rol: Capataz)
  - **Inicio:** 22/03/2026
  - **Fin:** 31/03/2026
  - **Duración:** 1.5 semanas
  - **Descripción:** Implementación del módulo WorkOrders orientado al rol Capataz para planificación de tareas agrícolas. Vista de planificador de órdenes con calendario o tabla de trabajo. Formulario de creación de orden de trabajo con selección de parcela (solo de campos gestionados por el capataz), fecha programada, tipo de tarea, descripción, y asignación a operario. Vista de listado de órdenes con filtros por estado, fecha, parcela, operario. Vista de detalle con timeline de actividades asociadas. Dashboard de órdenes pendientes vs. completadas. Notificaciones visuales de órdenes vencidas. Autorización: solo capataces y admins acceden. Integración con sistema de autorización por campos gestionados del backend.

### 22. F7: Módulo de Actividades para registro de ejecución (Rol: Operario)
  - **Inicio:** 01/04/2026
  - **Fin:** 10/04/2026
  - **Duración:** 1.5 semanas
  - **Descripción:** Desarrollo del módulo Activities orientado al rol Operario para registro de actividades ejecutadas. Vista de listado de órdenes de trabajo asignadas al operario autenticado. Formulario de registro de actividad con campos: fecha real de ejecución, estado (completada/en progreso), observaciones, registro fotográfico opcional. Submódulo de InputUsage para registrar insumos utilizados: selección de insumo, cantidad, unidad. Validación de stock disponible en frontend. Dashboard del operario con tareas del día, completadas, pendientes. Vista de historial de actividades propias. Interfaz simplificada y optimizada para uso móvil (campo). Documentación en módulos de activities.

### 23. F8: Módulo de Lotes de Cosecha con inventario visual
  - **Inicio:** 11/04/2026
  - **Fin:** 18/04/2026
  - **Duración:** 1 semana
  - **Descripción:** Implementación del módulo HarvestLots para gestión de inventario de producción. Vista de listado de lotes con tarjetas mostrando variedad, cantidad, fecha de cosecha, estado. Filtros por estado (en almacén, asignado, enviado), variedad, fecha. Formulario de creación de lote de cosecha con selección de parcelas de origen (múltiples), variedad, cantidad total, fecha, ubicación de almacenamiento. Vista de detalle de lote con trazabilidad: parcelas de origen, actividades de cosecha asociadas. Indicadores visuales de stock disponible vs. asignado. Dashboard de inventario con gráficos de cantidades por variedad y estado.

### 24. F9: Módulo de Ventas con asignación de lotes
  - **Inicio:** 19/04/2026
  - **Fin:** 26/04/2026
  - **Duración:** 1 semana
  - **Descripción:** Desarrollo del módulo Sales para gestión de órdenes de venta. Vista de listado de ventas con DataTable filtrable por cliente, estado, fecha. Formulario de creación de orden de venta con selección de cliente, agregado dinámico de ítems (variedad, cantidad, precio), asignación de lotes de cosecha disponibles a cada ítem con validación de stock. Cálculo automático de totales y subtotales. Vista de detalle de venta con información completa de lotes asignados. Estados visuales con badges. Endpoints de confirmación y cancelación de ventas.

### 25. F10: Módulo de Envíos con tracking de entregas
  - **Inicio:** 27/04/2026
  - **Fin:** 04/05/2026
  - **Duración:** 1 semana
  - **Descripción:** Implementación del módulo Shipments para gestión de despachos. Vista de listado de envíos con estado de entrega. Formulario de creación de envío vinculado a orden de venta, con selección de lotes específicos a enviar, información de transporte, guía de seguimiento, fecha estimada de entrega. Vista de detalle con timeline de estados: preparando → en tránsito → entregado. Actualización de estado de envío con registro de fecha real de entrega. Integración con ventas: marcado automático de venta como enviada. Dashboard de envíos en tránsito.

### 26. F11: Módulo de Reportes con visualizaciones y gráficos
  - **Inicio:** 05/05/2026
  - **Fin:** 12/05/2026
  - **Duración:** 1 semana
  - **Descripción:** Desarrollo del módulo Reports con visualizaciones interactivas usando librerías como Chart.js o Recharts. Páginas de reportes: producción por campo/período con gráficos de barras, uso de insumos con gráficos de torta, ventas con gráficos de líneas, inventario actual con tablas y cards. Filtros dinámicos por fechas, campos, clientes. Exportación de reportes a PDF o Excel. Dashboard gerencial con KPIs: producción total, costos, ganancias, lotes en inventario. Integración con endpoints de reportes del backend.

### 27. F12: Módulo de trazabilidad E2E con visualización de flujo
  - **Inicio:** 13/05/2026
  - **Fin:** 20/05/2026
  - **Duración:** 1 semana
  - **Descripción:** Implementación del módulo de trazabilidad completa con visualización tipo flowchart o timeline. Buscador de lotes de cosecha que muestra: origen (parcelas, actividades, insumos usados), destino (ventas, envíos, clientes). Visualización gráfica del flujo de trazabilidad con nodos interactivos. Vista inversa: dado un envío, mostrar toda la cadena hacia atrás. Integración con endpoints E2E del backend. Interfaz para cumplimiento de normativas de trazabilidad alimentaria.

### 28. F13: Empaquetado multiplataforma con Electron y Capacitor
  - **Inicio:** 21/05/2026
  - **Fin:** 28/05/2026
  - **Duración:** 1 semana
  - **Descripción:** Configuración y empaquetado de la aplicación para múltiples plataformas. Setup de Electron para compilar aplicación de escritorio para Windows, macOS, Linux. Configuración de Capacitor para compilar aplicación móvil para Android e iOS. Ajustes específicos de cada plataforma: íconos, splash screens, permisos (cámara para fotos en actividades). Pruebas de funcionalidad en cada plataforma. Generación de instaladores y APKs. Documentación de proceso de build y despliegue.
